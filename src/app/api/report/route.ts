import { generateReport, needsBroaderCatalog } from "@/features/quiz/services/geminiService";
import { buildReportPrompt } from "@/features/quiz/utils/buildReportPrompt";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (needsBroaderCatalog(report)) {
      return NextResponse.json(
        {
          error: "Could not build a report from available university catalogs",
          report: null,
          usedLayer,
        },
        { status: 422 },
